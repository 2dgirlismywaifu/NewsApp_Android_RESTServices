...
public class UserController {
  ...
  public ResponseEntity<UserDto> registerUser(
    ...
    String verify = "false";
    this.userLogin =
        new UserLogin(userIdRandom, email, userTokenHash, Salt, nickname, verify, recoveryCode, 0);
    this.userInformation =
        new UserInformation(userIdRandom, fullName, "not_input", this.date, "not_available", 0);
  ...
  public ResponseEntity<UserDto> signIn(
    ...
    this.userLogin =
        this.userServices
            .findByEmail(this.getDecode(email.getBytes(StandardCharsets.UTF_8)))
            .orElseThrow(() -> new ResourceNotFound("Failed"));
    this.userInformation =
        this.userServices
            .findInformationByUserId(String.valueOf(this.userLogin.getUserId()))
            .orElseThrow(() -> new ResourceNotFound("Failed"));
  ...
  public ResponseEntity<UserDto> signInByGoogle(
    ...
    UserDto userDto = new UserDto();
    Optional<UserLogin> userPassLoginOption =
        this.userServices.findByEmail(this.getDecode(email.getBytes()));
    if (userPassLoginOption.isPresent()) {
      ...
      if (userPassLoginOption.get().getUserToken().equals(tempUserTokenHash)) {
        this.userInformation =
            this.userServices
                .findInformationByUserId(String.valueOf(userPassLoginOption.get().getUserId()))
                .orElseThrow(() -> new ResourceNotFound("Failed"));
      ...
      this.userInformation =
          new UserInformation(
              userIdRandom,
              this.getDecode(fullName.getBytes()),
              "not_input",
              "not_input",
              this.getDecode(avatar.getBytes()),
              0);
      this.userServices.saveUser(this.userLogin);
      this.userServices.saveInformation(this.userInformation);
      userDto.setUserId(String.valueOf(this.userLogin.getUserId()));
  ...
  public ResponseEntity<HashMap<String, String>> CheckNickname(
    ...
    Optional<UserNameAndEmailDto> countNickNameOrEmail =
        this.userServices.countNickNameOrEmail(
            this.getDecode(nickname.getBytes(StandardCharsets.UTF_8)),
  ...
  public ResponseEntity<HashMap<String, String>> recoveryCode(
    ...
    this.userLogin =
        this.userServices.findByRecovery(email).orElseThrow(() -> new ResourceNotFound("Failed"));
  ...
  public ResponseEntity<HashMap<String, String>> changeUserToken(
      ...
      @RequestParam(name = "email") String email,
      @RequestParam(name = "newToken") String newToken)
      throws ResourceNotFound {
    this.userLogin =
        this.userServices
            .findByEmailOrUserid(
    ...
    String salt = this.userLogin.getSalt();
    String userTokenCheck =
        BCrypt.hashpw(this.getDecode(oldUserTokenHash.getBytes(StandardCharsets.UTF_8)), salt);
    // Bcrypt userToken
    String newSalt = BCrypt.gensalt();
    String newUserTokenHash =
        BCrypt.hashpw(this.getDecode(newToken.getBytes(StandardCharsets.UTF_8)), newSalt);
    if (userTokenCheck.equals(oldUserTokenHash)) {
      this.userServices.updateUserToken(
          newUserTokenHash,
          newSalt,
          recoveryCode,
          this.getDecode(email.getBytes(StandardCharsets.UTF_8)));
  ...
  public ResponseEntity<UserDto> updateUserInformation(
    ...
    if (username != null && !username.isEmpty()) {
      this.userServices.updateNickname(
          this.getDecode(username.getBytes(StandardCharsets.UTF_8)),
          this.getDecode(userid.getBytes(StandardCharsets.UTF_8)));
      userDto.setStatus("success");
    ...
    if (fullName != null && !fullName.isEmpty()) {
      this.userServices.updateFullName(
          this.getDecode(fullName.getBytes(StandardCharsets.UTF_8)),
          this.getDecode(userid.getBytes(StandardCharsets.UTF_8)));
      userDto.setStatus("success");
    ...
    if (birthday != null && !birthday.isEmpty()) {
      this.userServices.updateBirthday(
          this.getDecode(birthday.getBytes(StandardCharsets.UTF_8)),
          this.getDecode(userid.getBytes(StandardCharsets.UTF_8)));
      userDto.setStatus("success");
    ...
    if (gender != null && !gender.isEmpty()) {
      this.userServices.updateGender(
          this.getDecode(gender.getBytes(StandardCharsets.UTF_8)),
          this.getDecode(userid.getBytes(StandardCharsets.UTF_8)));
      userDto.setStatus("success");
    ...
    if (avatar != null && !avatar.isEmpty()) {
      this.userServices.updateAvatar(
          this.getDecode(avatar.getBytes(StandardCharsets.UTF_8)),
          this.getDecode(userid.getBytes(StandardCharsets.UTF_8)));
      userDto.setStatus("success");
  ...
  @GetMapping(
      value = "/verify-recovery-code",
      params = {"code"})
  public ResponseEntity<UserDto> recoveryAccount(@RequestParam(name = "code") String code)
      throws ResourceNotFound {
    UserDto userDto = new UserDto();
    this.userLogin =
        this.userServices
            .findByRecovery(this.getDecode(code.getBytes(StandardCharsets.UTF_8)))
            .orElseThrow(() -> new ResourceNotFound("Failed"));
    this.userInformation =
        this.userServices
            .findInformationByUserId(String.valueOf(this.userLogin.getUserId()))
