...
@Tag(
    name = "Standard Account Favourite Controller",
    description = "API for Standard Account Favourite Controller")
public class UserFavouriteController {
  ...
  @Autowired private SyncServices syncServices;
  ...
  public ResponseEntity<HashMap<String, String>> userSourceSubscribe(
    ...
    this.syncSubscribe =
        new SyncSubscribe(
            Integer.parseInt(syncIdRandom),
            Integer.parseInt(userId),
            Integer.parseInt(source_id),
            0,
            1);
    ...
    return new ResponseEntity<>(
        new HashMap<>() {
          {
            this.put(
                "syncId", String.valueOf(UserFavouriteController.this.syncSubscribe.getSyncId()));
            this.put(
                "userId", String.valueOf(UserFavouriteController.this.syncSubscribe.getUserId()));
            this.put("isChecked", "1");
            this.put(
                "sourceId",
                String.valueOf(UserFavouriteController.this.syncSubscribe.getSourceId()));
  ...
  public ResponseEntity<HashMap<String, String>> userNewsFavourite(
    ...
    long total =
        this.syncServices.findByNewsFavourite(
            this.getDecode(userId.getBytes()), this.getDecode(url.getBytes()));
    if (total > 0) {
      respond.put("status", "found");
      return new ResponseEntity<>(respond, HttpStatus.BAD_REQUEST);

    } else {
      this.syncNewsFavourite =
          new SyncNewsFavourite(
              Integer.parseInt(favouriteIdRandom),
              Integer.parseInt(this.getDecode(userId.getBytes())),
              this.getDecode(url.getBytes()),
              this.getDecode(title.getBytes()),
              this.getDecode(imageUrl.getBytes()),
              this.getDecode(pubDate.getBytes()),
              0);
      this.syncServices.saveNewsFavourite(this.syncNewsFavourite);
      respond.put(
          "favouriteId",
          String.valueOf(UserFavouriteController.this.syncNewsFavourite.getFavouriteId()));
      respond.put(
          "userId", String.valueOf(UserFavouriteController.this.syncNewsFavourite.getUserId()));
  ...
  public ResponseEntity<HashMap<String, String>> userNewsFavouriteDelete(
    ...
    if (favouriteId == null || favouriteId.isEmpty()) {
      favouriteId =
          this.syncServices.findFavoriteId(
              this.getDecode(userId.getBytes()), this.getDecode(title.getBytes()));
    }
    this.syncServices.deleteNewsFavourite(
        this.getDecode(userId.getBytes()), this.getDecode(favouriteId.getBytes()));
        ...
        new HashMap<>() {
          {
            ...
            this.put(
                "favouriteId", UserFavouriteController.this.getDecode(finalFavouriteId.getBytes()));
            this.put("status", "deleted");
          }
        },
        org.springframework.http.HttpStatus.OK);
  ...
  public ResponseEntity<NewsFavouriteDto> userNewsFavouriteShow(
    ...
    List<SyncNewsFavourite> syncNewsFavourites =
        this.syncServices.findByUserId(
            Integer.parseInt(UserFavouriteController.this.getDecode(userId.getBytes())));
    String totalResults = String.valueOf(syncNewsFavourites.size());
    if (syncNewsFavourites.isEmpty()) {
      return new ResponseEntity<>(
          new NewsFavouriteDto(
              "not-found",
  ...
  // Check source news in SYNC_SUBSCRIBE table. use params: userId, source_id. DO NOT USE
  // SYNC_NEWS_FAVOURITE TABLE
  ...
  public ResponseEntity<HashMap<String, String>> userSourceSubscribeCheck(
    ...
    HashMap<String, String> respond = new HashMap<>();
    this.syncSubscribe =
        this.syncServices.findByUserIdAndSourceId(
            Integer.parseInt(this.getDecode(userId.getBytes())),
            this.getDecode(source_id.getBytes()));
    if (this.syncSubscribe == null) {
      ...
      respond.put(
          "sourceId", String.valueOf(UserFavouriteController.this.syncSubscribe.getSourceId()));
      respond.put(
          "isChecked", String.valueOf(UserFavouriteController.this.syncSubscribe.getIsChecked()));
      respond.put(
          "isDeleted", String.valueOf(UserFavouriteController.this.syncSubscribe.getIsDeleted()));
      return new ResponseEntity<>(respond, HttpStatus.OK);
    }
  }
  ...
  public ResponseEntity<HashMap<String, String>> userNewsFavouriteCheck(
      @RequestParam(value = "userId") String userId, @RequestParam(value = "title") String title) {
    HashMap<String, String> respond = new HashMap<>();
    this.syncNewsFavourite =
        this.syncServices.checkNewsFavouriteOrNot(
            this.getDecode(userId.getBytes()), this.getDecode(title.getBytes()));
