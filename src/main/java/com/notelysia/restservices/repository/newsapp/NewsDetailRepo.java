...
public interface NewsDetailRepo extends JpaRepository<NewsDetail, Long> {
  ...
  // Query example: SELECT * FROM NEWS_DETAIL, NEWS_SOURCE WHERE NEWS_DETAIL.source_id =
  // NEWS_SOURCE.source_id AND url_type = ? AND NEWS_SOURCE.source_name=?
  @Query(
      "SELECT n FROM NewsDetail n, NewsSource ns WHERE n.sourceId = ns.sourceId AND n.urlType = ?1"
          + " AND ns.source_name = ?2")
  List<NewsDetail> findByUrlTypeAndSourceName(String urlType, String sourceName);

  // This is list url for each source
  // Query example: "SELECT * FROM NEWS_DETAIL, NEWS_SOURCE WHERE NEWS_DETAIL.source_id =
  // NEWS_SOURCE.source_id AND NEWS_SOURCE.source_name=?"
  @Query(
      "SELECT n FROM NewsDetail n, NewsSource ns WHERE n.sourceId = ns.sourceId AND ns.source_name"
          + " = ?1")
  List<NewsDetail> findBySourceName(String sourceName);

  // GET URL RSS LIST FOLLOW SOURCE_NAME
  @Query(
      "SELECT  new com.notelysia.restservices.model.dto.newsapp.RssList(n.urlType, n.url,"
          + " ni.urlImage) FROM NewsDetail n, NewsSource ns, NewsTypeImage ni WHERE n.urlType ="
          + " ni.urlType AND n.sourceId = ns.sourceId AND ns.source_name = ?1")
  List<RssList> findUrlBySourceName(String sourceName);

  @Query(
      "select nd.url from NewsDetail nd "
