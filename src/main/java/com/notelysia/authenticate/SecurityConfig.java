...
public class SecurityConfig {
  ...
  FileInputStream in;
  @Autowired private AuthApiKeyServices authApiKeyServices;
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) {
    try {
      ...
      this.in.close();
      String newsAppHeader =
          new String(
              Base64.getDecoder().decode(this.props.getProperty("auth-token-header-news-app")));
      String bookStoreHeader =
          new String(
              Base64.getDecoder().decode(this.props.getProperty("auth-token-header-bookstore")));
      ...
      http.authorizeHttpRequests(
              (request ->
                  request
                      .requestMatchers(antMatcher("/news-api/**"), antMatcher("/news-app/**"))
                      .authenticated()))
          .addFilter(newsApp)
          .sessionManagement(
              sessionManagement ->
      ...
      http.authorizeHttpRequests(
              (request -> request.requestMatchers(antMatcher("/book-store/**")).authenticated()))
          .addFilter(bookStore)
          .sessionManagement(
              sessionManagement ->
                  sessionManagement.sessionCreationPolicy(SessionCreationPolicy.STATELESS));
      ...
      http.authorizeHttpRequests(
          (request ->
              request
                  ...
                  requestMatchers(antMatcher("/legacy-key-generator/**"))
                  .permitAll()
                  .
